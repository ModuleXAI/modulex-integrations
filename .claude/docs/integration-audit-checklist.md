...
| 8.10 | Every `response.json()` is routed through an `_as_dict()`-style guard (or an explicit `isinstance` branch) before any `.get()` | `data = response.json()` is the last statement in `try:` while its consumers sit after the `except` clauses, so a 200 with a bare-array/string/null body raises `AttributeError` through the `@tool` boundary — the exact failure the `success=False` envelope exists to prevent. Mocked tests cannot catch it: fixtures always return objects. **Probe it** with a `MockTransport` returning a bare array. | grep for `= response.json()`; drive one list action through a transport returning `[]` | FIX | safe-semantic (wrap each parse in `_as_dict(...)`; leave deliberately polymorphic actions with their explicit `isinstance` branch) |
