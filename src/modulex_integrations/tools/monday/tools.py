...
class CreateBoardInput(BaseModel):
    ...
    api_key: str = Field(description="Monday.com API token")
...
async def create_board(
    ...
    if not api_key or not api_key.strip():
        return CreateBoardOutput(success=False, error="API key is empty. Please configure a valid credential.")
...
async def create_column(
    ...
    if not api_key or not api_key.strip():
        return CreateColumnOutput(success=False, error="API key is empty. Please configure a valid credential.")
...
async def create_group(
    ...
    if not api_key or not api_key.strip():
        return CreateGroupOutput(success=False, error="API key is empty. Please configure a valid credential.")
    query = (
...
async def create_item(
    ...
    if not api_key or not api_key.strip():
        return CreateItemOutput(success=False, error="API key is empty. Please configure a valid credential.")
...
async def create_subitem(
    ...
    if not api_key or not api_key.strip():
        return CreateSubitemOutput(success=False, error="API key is empty. Please configure a valid credential.")
...
async def create_update(
    ...
    if not api_key or not api_key.strip():
        return CreateUpdateOutput(success=False, error="API key is empty. Please configure a valid credential.")
...
async def get_board_items_page(
    ...
    if not api_key or not api_key.strip():
        return GetBoardItemsPageOutput(success=False, error="API key is empty. Please configure a valid credential.")
    query_args = ""
    if query_params:
    ...
    try:
        ...
        result = await _graphql(api_key, query)
        error = _check_errors(result)
        if error:
        ...
        if boards:
            ...
            while cursor and pages_seen < max_pages:
                ...
                result = await _graphql(api_key, next_query)
                error = _check_errors(result)
                if error:
                    break
...
async def get_column_values(
    ...
    if not api_key or not api_key.strip():
        return GetColumnValuesOutput(success=False, error="API key is empty. Please configure a valid credential.")
    col_filter = ""
    if column_ids:
...
async def get_items_by_column_value(
    ...
    if not api_key or not api_key.strip():
        return GetItemsByColumnValueOutput(success=False, error="API key is empty. Please configure a valid credential.")
    safe_board_id = json.dumps(board_id)
...
async def list_boards(
    ...
    if not api_key or not api_key.strip():
        return ListBoardsOutput(success=False, error="API key is empty. Please configure a valid credential.")
...
async def update_column_values(
    ...
    if not api_key or not api_key.strip():
        return UpdateColumnValuesOutput(success=False, error="API key is empty. Please configure a valid credential.")
    query = (
...
async def update_item_name(
    ...
    if not api_key or not api_key.strip():
        return UpdateItemNameOutput(success=False, error="API key is empty. Please configure a valid credential.")
