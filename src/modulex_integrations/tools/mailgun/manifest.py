    ...
    auth_schemas=[
        ApiKeyAuthSchema(
            ...
            test_endpoint=TestEndpoint(
                ...
                method="GET",
                # Mailgun requires HTTP Basic Auth with literal "api" as
                # the username and the API key as the password. The
                # modulex runtime synthesises the
                # ``Authorization: Basic <base64(api:KEY)>`` header from
                # this directive.
                auth=BasicAuthSpec(
                    username_placeholder="api",
                ...
                cost_level="free",
                description="Lists domains to validate the API key via Basic Auth.",
            ),
        ),
    ],
)
