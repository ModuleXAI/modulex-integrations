...
class TestManifest:
    ...
    def test_test_endpoint_embeds_key_in_url(self) -> None:
        # ``params`` placeholders are NOT substituted by the modulex runtime
        # — the ``key`` credential lives in the URL query string instead.
        auth = manifest.auth_schemas[0]
        assert auth.test_endpoint is not None
        assert auth.test_endpoint.params == {}
        assert "key={api_key}" in auth.test_endpoint.url
        assert "type=api_units" in auth.test_endpoint.url
